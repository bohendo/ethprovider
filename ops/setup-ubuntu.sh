...
project="ethprovider"
repo="https://gitlab.com/bohendo/ethprovider.git"
user="ubuntu"
...
if [[ -f "$aliases" ]]
then
  echo "Copying $aliases to remote server..."
  scp -i "$prvkey" "$aliases" "$user@$hostname:/home/$user/.bash_aliases"
fi

echo "Setting up $user@$hostname"
# shellcheck disable=SC2087
